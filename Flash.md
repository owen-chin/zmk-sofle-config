...
1. Connect that half to your Mac via USB-C **directly** (not through the other half).
2. Press the reset button on the side.
   - Try a single press first.
   - If no drive appears, try a quick double-press instead.
3. Confirm the drive mounted:
```bash
   ls /Volumes/
```
...
```bash
   cp ~/Downloads/<firmware-folder>/left.uf2 /Volumes/NICENANO/; sync
```
